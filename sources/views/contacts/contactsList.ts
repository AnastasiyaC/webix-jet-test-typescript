...
export default class ContactsList extends JetView implements IcontactsList {
	...
	init(): void {
		...
		contactsCollection.waitData.then(() => {
			...
			const listFirstId = this.list.getFirstId() as string;
			...
			this.list.select(listFirstId, false);
		});
		...
		this.on(contactsCollection.data, "onAfterDelete", () => this.list.select(this.list.getFirstId() as string, false));
		this.on(this.listFilter, "onTimedKeyPress", () => {
			this.filterList();
			if (!this.list.getFirstId()) this.show("./contacts.contactInfo");
			this.list.select(this.list.getFirstId() as string, false);
		});
		...
		this.on(this.app, "contactForm:close", (id: string) => {
			...
			else this.list.select(id || this.list.getFirstId() as string, false);
			this.filterList();
			this.listFilter.enable();
		});
	}
